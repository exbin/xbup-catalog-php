...
//    echo "parent: ".$node['id']." xbindex: ".$var_spec."\n";
 ...
 case "getinfo": {
  ...
  case "getline": {
    ...
    $var_rev += 0;
    $var_prior += 0;
    $line = DB_SimpleQuery('SELECT item_line.id AS id, plug_line.plugin_id AS plugin_id, item_line.line_id AS lineid, item_line.*, plug_line.*, plug.* FROM XBXBLOCKLINE item_line LEFT JOIN XBXPLUGLINE plug_line ON plug_line.id = item_line.line_id LEFT JOIN XBXPLUGIN plug ON plug.id = plug_line.plugin_id WHERE blockrev_id = '.$var_rev.' AND priority='.$var_prior);
    if ($line) {
      if (!$var_debug) echo "id\n".$line['id']."\nlineid\n".$line['lineid']."\nline\n".$line['line']."\nplug\n".$line['LINEINDEX']."\nplugin\n".$line['PLUGININDEX']."\nplugnode\n".$line['OWNER_ID']."\n";
    }
    break;
